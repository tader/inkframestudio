...
use routes::fonts::{
    dafont_page, delete_font, import_dafont, import_font, list_fonts, rescan_fonts,
...
pub(crate) struct DaFontEntry {
    ...
    preview_url: String,
    #[serde(rename = "pixelSize", skip_serializing_if = "Option::is_none")]
    pixel_size: Option<u32>,
    #[serde(rename = "licenseCategory", skip_serializing_if = "Option::is_none")]
    license_category: Option<String>,
    #[serde(rename = "downloadSizeLabel", skip_serializing_if = "Option::is_none")]
    download_size_label: Option<String>,
}

#[derive(Debug, Clone, Serialize, Deserialize, utoipa::ToSchema)]
pub(crate) struct DaFontPageResponse {
    page: u32,
    ...
    has_next_page: bool,
    entries: Vec<DaFontEntry>,
}

#[derive(Debug, Deserialize)]
pub(crate) struct DaFontPageQuery {
    page: Option<u32>,
}
...
fn app(state: AppState) -> Router {
    ...
    Router::new()
        ...
        .route("/api/v2/dafont", get(dafont_page))
        .route("/api/v2/dafont/import", post(import_dafont))
        .route(
...
const DAFONT_BASE_URL: &str = "https://www.dafont.com";
const DAFONT_BITMAP_URL: &str = "https://www.dafont.com/bitmap.php";
...
fn normalize_whitespace(value: &str) -> String {
    Regex::new(r"\s+")
        ...
        .replace_all(&decode_html(&strip_tags(value)), " ")
        .trim()
        .to_string()
}

pub(crate) fn absolute_dafont_url(value: &str) -> String {
    if value.is_empty() {
        return String::new();
    }
    ...
    format!(
        "{}/{}",
        DAFONT_BASE_URL.trim_end_matches('/'),
        value.trim_start_matches('/')
    )
}

fn safe_id(value: &str) -> String {
    slugify_font_id(value)
}

fn parse_pixel_size(value: &str) -> Option<u32> {
    Regex::new(r"\((\d+)\s*px\)")
        .expect("valid pixel size regex")
        .captures(value)
        .and_then(|captures| captures.get(1))
        .and_then(|value| value.as_str().parse::<u32>().ok())
}

fn parse_download_size_label(value: &str) -> Option<String> {
    Regex::new(r#"title="([^"]+)""#)
...
fn family_match_score(expected: &str, actual: &str, filename: &str) -> i32 {
    let expected_name = normalize_name(expected);
    let actual_name = normalize_name(actual);
    let filename_name = normalize_name(filename.trim_end_matches(|character| character != '.'));
    if expected_name.is_empty() {
        return 0;
    }
    if actual_name == expected_name || filename_name == expected_name {
        return 100;
    }
    if actual_name.contains(&expected_name) || expected_name.contains(&actual_name) {
        return 80;
    }
    if filename_name.contains(&expected_name) || expected_name.contains(&filename_name) {
        return 75;
    }
    let expected_lower = expected.to_lowercase();
    let expected_tokens = expected_lower
        .split(|character: char| !character.is_ascii_alphanumeric())
        .filter(|token| !token.is_empty())
        .collect::<Vec<_>>();
    let actual_lower = actual.to_lowercase();
    let actual_tokens = actual_lower
        .split(|character: char| !character.is_ascii_alphanumeric())
        .filter(|token| !token.is_empty())
        .collect::<Vec<_>>();
    expected_tokens
...
pub(crate) fn assert_dafont_url(value: &str) -> Result<(), ApiError> {
    let url = reqwest::Url::parse(&absolute_dafont_url(value))
        .map_err(|error| ApiError::bad_request(format!("Invalid DaFont URL: {error}")))?;
    let host = url.host_str().unwrap_or_default();
    if matches!(host, "dafont.com" | "www.dafont.com" | "dl.dafont.com") {
        Ok(())
    } else {
        Err(ApiError::bad_request(format!("Unsupported DaFont host: {host}")))
    }
}

fn detect_font_variant_from_subfamily(value: Option<&str>) -> String {
...
pub(crate) fn build_allowed_sizes(base_size: Option<u32>) -> Option<Vec<i32>> {
    ...
    Some(
        (base_size..=200)
            .step_by(base_size as usize)
            .map(|size| size as i32)
            .collect(),
    )
}

pub(crate) fn extract_font_names(bytes: &[u8], fallback_filename: &str) -> (String, String) {
    if let Ok(face) = ttf_parser::Face::parse(bytes, 0) {
        let mut family = None;
        let mut subfamily = None;
        for name in face.names() {
            match name.name_id {
                ttf_parser::name_id::FULL_NAME | ttf_parser::name_id::TYPOGRAPHIC_FAMILY | ttf_parser::name_id::FAMILY => {
                    if family.is_none() {
        ...
        return (
            family.unwrap_or_else(|| fallback_filename.to_string()),
            detect_font_variant_from_subfamily(subfamily.as_deref()),
        );
    }
    (
        fallback_filename
            ...
            .replace(['_', '-'], " "),
        detect_font_variant_from_name(fallback_filename),
    )
}

pub(crate) fn choose_best_variants(files: &[(String, Vec<u8>, String, String, i32)]) -> Vec<(String, Vec<u8>, String, String, i32)> {
    let mut by_variant = std::collections::BTreeMap::<String, (String, Vec<u8>, String, String, i32)>::new();
    for file in files {
        let variant = file.3.clone();
        let replace = by_variant
            .get(&variant)
            .map(|existing| file.4 < existing.4)
            .unwrap_or(true);
        if replace {
            by_variant.insert(variant, file.clone());
        }
    }
    by_variant.into_values().collect()
}

pub(crate) fn choose_best_family(files: &[(String, Vec<u8>, String, String, i32)], expected_name: &str) -> Vec<(String, Vec<u8>, String, String, i32)> {
    let mut groups = std::collections::BTreeMap::<String, Vec<(String, Vec<u8>, String, String, i32)>>::new();
    for file in files {
        groups.entry(file.2.clone()).or_default().push(file.clone());
    }
    groups
        .into_iter()
        .max_by(|(left_label, left_group), (right_label, right_group)| {
            let left_score = left_group
                .iter()
                .map(|file| family_match_score(expected_name, left_label, &file.0))
                .max()
                .unwrap_or(0);
            let right_score = right_group
                .iter()
                .map(|file| family_match_score(expected_name, right_label, &file.0))
                .max()
                .unwrap_or(0);
            left_score
                .cmp(&right_score)
                .then_with(|| {
                    let left_rank = left_group.iter().map(|file| file.4).min().unwrap_or(i32::MAX);
                    let right_rank = right_group.iter().map(|file| file.4).min().unwrap_or(i32::MAX);
...
fn parse_dafont_bitmap_page(html: &str, page: u32) -> DaFontPageResponse {
    let entry_pattern = Regex::new(r#"(?si)<a name="[^"]*"></a><div class="lv1left dfbg">(.*?)</div><div class="lv1right dfbg">(.*?)</div><div class="lv2right">(.*?)</div><div class="dlbox".*?>(.*?)</div>((?:<div style="background-image:url\([^)]+\).*?class="preview">.*?</div>)+)"#)
        .expect("valid dafont entry regex");
    let detail_pattern = Regex::new(r#"(?si)<a href="([^"]+?\.font(?:\?[^"]*)?)"><strong>(.*?)</strong></a>"#).expect("valid detail regex");
    let author_pattern = Regex::new(r#"(?si)by <a [^>]*>(.*?)</a>"#).expect("valid author regex");
    let download_pattern = Regex::new(r#"(?si)<a class="dl"[^>]*href="([^"]+)""#).expect("valid download regex");
    let preview_pattern = Regex::new(r#"(?si)background-image:url\(([^)]+)\)"#).expect("valid preview regex");
    let license_pattern = Regex::new(r#"(?si)<a[^>]*faq\.php#copyright[^>]*>(.*?)</a>"#).expect("valid license regex");
    let pages_pattern = Regex::new(r#"bitmap\.php\?page=(\d+)&af=on"#).expect("valid page regex");
    let mut entries = Vec::new();

    for captures in entry_pattern.captures_iter(html) {
        let left_html = captures.get(1).map(|value| value.as_str()).unwrap_or_default();
        let right_html = captures.get(2).map(|value| value.as_str()).unwrap_or_default();
        let meta_html = captures.get(3).map(|value| value.as_str()).unwrap_or_default();
        let download_html = captures.get(4).map(|value| value.as_str()).unwrap_or_default();
        let preview_html = captures.get(5).map(|value| value.as_str()).unwrap_or_default();
        let Some(detail_match) = detail_pattern.captures(left_html) else { continue; };
        let Some(download_match) = download_pattern.captures(download_html) else { continue; };
        let Some(preview_match) = preview_pattern.captures(preview_html) else { continue; };
        let detail_url = absolute_dafont_url(detail_match.get(1).map(|value| value.as_str()).unwrap_or_default());
        let name = normalize_whitespace(detail_match.get(2).map(|value| value.as_str()).unwrap_or_default());
        let download_url = absolute_dafont_url(download_match.get(1).map(|value| value.as_str()).unwrap_or_default());
        let preview_url = absolute_dafont_url(preview_match.get(1).map(|value| value.as_str()).unwrap_or_default().trim_matches(&['"', '\''][..]));
        let author = author_pattern
            .captures(left_html)
            .and_then(|captures| captures.get(1))
            .map(|value| normalize_whitespace(value.as_str()))
            .filter(|value| !value.is_empty());
        let license_category = license_pattern
            .captures(meta_html)
            .and_then(|captures| captures.get(1))
            .map(|value| normalize_whitespace(value.as_str()))
            .filter(|value| !value.is_empty());
        entries.push(DaFontEntry {
            id: safe_id(if name.is_empty() { &detail_url } else { &name }),
            name,
            author,
            detail_url,
            download_url,
            preview_url,
            pixel_size: parse_pixel_size(right_html),
            license_category,
            download_size_label: parse_download_size_label(download_html),
        });
    }

    let mut total_pages = page.max(1);
    for captures in pages_pattern.captures_iter(html) {
        if let Some(value) = captures.get(1).and_then(|value| value.as_str().parse::<u32>().ok()) {
            total_pages = total_pages.max(value);
        }
    }
    DaFontPageResponse {
        page,
...
pub(crate) async fn fetch_dafont_page_native(
    client: &reqwest::Client,
    page: u32,
) -> Result<DaFontPageResponse, ApiError> {
    let page = page.max(1);
    let response = client
        .get(format!("{DAFONT_BITMAP_URL}?page={page}&fpp=200&af=on"))
    ...
    if !response.status().is_success() {
        return Err(ApiError::bad_request(format!(
            "DaFont page request failed with {}",
            response.status().as_u16()
        )));
    }
    Ok(parse_dafont_bitmap_page(
